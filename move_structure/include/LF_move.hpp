...
public:
  ...
  LF_row() {}
  LF_row(uchar c, ulint l, ulint p, ulint o)
      : character(c), length(l), pointer(p), offset(o) {}
...
public:
  ...
  LF_row_raw() {}
  LF_row_raw(uchar c, ulint l, ulint p, ulint o)
      : character(c), length(l), pointer(p), offset(o) {}
};
...
template <typename row_t = LF_row_raw> class LF_move {
public:
  LF_move() {}

  // Expects two files: one with characters (BYTE) and one with lengths (5
  // BYTES)
  LF_move(std::ifstream &heads, std::ifstream &lengths) {
    ...
    vector<vector<size_t>> L_block_indices =
        vector<vector<size_t>>(ALPHABET_SIZE);
  ...
  uchar get_char(size_t i) { return get(i).character; }

  uchar get_char(position p) { return get(p).character; }
  ...
  std::string get_file_extension() const { return ".LF_move"; }
