...
int forward_backward(fm_index *fm, int64_t start, char *pattern, size_t pattern_size, int8_t direction) {
    int64_t s = 0;
 ...
 */
int find_mems(
    fm_index *fm_index_straight, fm_index *fm_index_reversed,
    ...
    while (mem_end <= pattern_size-1) {
        int64_t steps_fw = forward_backward(fm_index_reversed, mem_start, pattern, pattern_size, FORWARD);
        mem_end = mem_start + steps_fw;

        if (steps_fw >= MIN_MEM_LENGTH) {
            printf("MEM (start=%ld, end=%ld, len=%ld)\n", mem_start, mem_end, steps_fw);
        }
        ...
        int64_t steps_bw = forward_backward(fm_index_straight, mem_end, pattern, pattern_size, BACKWARD);
