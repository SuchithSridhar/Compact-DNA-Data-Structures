...
void build_BL(char *BWT, int n) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    // increase count of the characters to find the characters that exist in the
    // BWT
...
// We need the C that corresponds to B_F
// for doing LF in this build file. But, we
// need C on the H_F array for normal LF.
void build_HF_C() {
  int counts[sigma] = {0};

  for (int i = 0; i < r; i++) {
    counts[char_to_index[H_L[i]]]++;
  }
...
void build_B_FL() {
  int boundary_F = 1, boundary_L = 1;

  int pos_F = select_B_F(boundary_F);
  int pos_L = select_B_L(boundary_L);

  for (int i = 0; i < 2 * r; ++i) {
    if (pos_L <= pos_F) {
      B_FL[i] = 0;
      ++boundary_L;
      pos_L = boundary_L <= r ? select_B_L(boundary_L) : n;
    } else {
      B_FL[i] = 1;
      ++boundary_F;
      pos_F = boundary_F <= r ? select_B_F(boundary_F) : n;
    }
  }
}
...
int main(int argc, char **argv) {
  ...
  std::cerr << "C: ";
  std::for_each(C.begin(), C.end(), [](int i) { std::cerr << i << " "; });
