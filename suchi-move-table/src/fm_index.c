...
int init_symbols(fm_index *fm) {
    ...
    if (!fm->symbol_indices)
        return FM_INIT_MEM_ERR;

    for (size_t i = 0; i < SYMBOLS_MAX; i++)
        fm->symbol_indices[i] = -1;

    fm->symbols_len = 0;

    for (size_t i = 0; i < fm->bwt_length; i++) {
        char c = fm->bwt[i];
        if (fm->symbol_indices[(int)c] == -1 && c != DOLLAR_SIGN) {
            fm->symbol_indices[(int)c] = 1;
            fm->symbols_len++;
        }
    }
...
int init_bitvectors(fm_index *fm) {
    ...
    long miniheader_size =
        (fm->bwt_length + (MINIHEADER_SIZE - 1)) / MINIHEADER_SIZE;
    long macroheader_size =
        (fm->bwt_length + (MACROHEADER_SIZE - 1)) / MACROHEADER_SIZE;
    ...
    for (size_t i = 0; i < fm->symbols_len; i++) {
        ...
        fm->bitvectors[i].miniheader =
            calloc(miniheader_size, sizeof(uint16_t));
        fm->bitvectors[i].macroheader =
            calloc(macroheader_size, sizeof(uint64_t));

        if (!fm->bitvectors[i].bits || !fm->bitvectors[i].miniheader ||
            !fm->bitvectors[i].macroheader) {
            return FM_INIT_MEM_ERR;
        }
    }
    ...
    for (size_t i = 0; i < fm->bwt_length; i++) {
        ...
        if (i % MINIHEADER_SIZE == 0) {
            // set all miniheaders
            for (size_t j = 0; j < fm->symbols_len; j++) {
                fm->bitvectors[j].miniheader[i / MINIHEADER_SIZE] =
                    mini_ranks[j];
            }
        }
        ...
        int8_t index = fm->symbol_indices[(int)fm->bwt[i]];
        if (index == -1) {
...
int fm_index_init(fm_index *fm, char *bwt_filename) {
    ...
    if (!file)
        return FM_INIT_FILE_ERR;

    fm->bwt_length = get_file_length(file);
    if (fm->bwt_length == -1)
        return FM_INIT_BWT_LEN_ERR;
...
int64_t fm_index_rank(fm_index *fm, char character, int position) {
    ...
    int8_t index = fm->symbol_indices[(int)character];
    ...
    int64_t mini_value =
        (int64_t)fm->bitvectors[index].miniheader[position / MINIHEADER_SIZE];
    int64_t macro_value =
        (int64_t)fm->bitvectors[index].macroheader[position / MACROHEADER_SIZE];

    sum = mini_value + macro_value;

    uint64_t miniblock_chunk = *(uint64_t *)&(
        fm->bitvectors[index].bits)[(position / MINIHEADER_SIZE) * BYTE_SIZE];
...
int64_t fm_index_count_occur(fm_index *fm, char *query) {
    ...
    for (int64_t i = m - 1; i >= 0; i--) {
        int8_t index = fm->symbol_indices[(int)query[i]];
        if (index == -1) {
            // symbol not found
            return -1;
        }
...
void fm_index_destroy(fm_index *fm) {
    ...
    if (fm->symbols)
        free(fm->symbols);
    if (fm->symbol_indices)
        free(fm->symbol_indices);
    if (fm->symbol_counts)
        free(fm->symbol_counts);
    if (fm->symbol_cumulative_counts)
        free(fm->symbol_cumulative_counts);
