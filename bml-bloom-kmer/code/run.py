...
for pattern_num in range(0, len(patterns)):
    ...
    mems = subprocess.run(["./build/move_mem_finder", "data/dataset1.mvt", "data/tesatad1.mvt", 
                                f'data/{patterns[pattern_num]}', f'{mem_size}'], capture_output=True, text=True)
    ...
    for i in range(0, len(k)):
        _ = subprocess.run(["./build/build_kmer_filter", "data/dataset.txt", 
                            f'tmp/dataset_{k[i]}_{insPar[i]}.blm', f'{k[i]}', "500000", f'{insPar[i]}'])

        with open(f'tmp/substring_{k[i]}_{insPar[i]}.txt', "w") as output_file: 
            _ = subprocess.run(["./build/kmer_substrings", f'data/{patterns[pattern_num]}', f'tmp/dataset_{k[i]}_{insPar[i]}.blm', f'{mem_size}'], 
                           stdout=output_file, text=True)
        
        mems = subprocess.run(["./build/move_mem_finder", "data/dataset1.mvt",
                                 "data/tesatad1.mvt", f'tmp/substring_{k[i]}_{insPar[i]}.txt', f'{mem_size}'], capture_output=True, text=True)
